...
#include "mdnie_lite_table_degas.h"
#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!IS_ERR_OR_NULL(table) && !IS_ERR_OR_NULL(table->name)) {
		...
		pos += sprintf(pos, "\n");
		if (MDNIE_CMD1 != MDNIE_CMD6) {
			for (i = 0; i < table->tune[MDNIE_CMD6].size; i++)
				pos += sprintf(pos, "0x%02x ", table->tune[MDNIE_CMD6].sequence[i]);
		}
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	for (j = MDNIE_CMD1; j <= MDNIE_CMD6; j++) {
