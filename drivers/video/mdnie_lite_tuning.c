...
struct mdnie_table *mdnie_request_table(char *path, struct mdnie_table *s)
{
	...
	if (ret > 0) {
		for (j = 1, i = MDNIE_CMD1; i <= MDNIE_CMD6; i++, j++) {
		...
		t->tune[MDNIE_CMD3].sequence = &buf[i+1];
		t->tune[MDNIE_CMD3].size = size - i + 2;
		t->tune[MDNIE_CMD4].sequence = &buf[i+2];
		t->tune[MDNIE_CMD4].size = size - i + 3;
		t->tune[MDNIE_CMD5].sequence = &buf[i+3];
		t->tune[MDNIE_CMD5].size = size - i + 4;
		t->tune[MDNIE_CMD6].sequence = &buf[i+4];
		t->tune[MDNIE_CMD6].size = size - i + 5;
	}
