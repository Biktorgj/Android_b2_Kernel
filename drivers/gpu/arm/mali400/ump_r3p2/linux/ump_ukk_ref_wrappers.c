...
int ump_ion_import_wrapper(u32 __user * argument, struct ump_session_data  * session_data)
{
	...
	sgtable_ion = ion_sg_table(ion_client_ump, ion_hnd);

	if (NULL == sgtable_ion) {
		MSG_ERR(("Failed to get sg_table in ump_ioctl_allocate()\n"));
		return -EFAULT;
	}
	sg_ion = sgtable_ion->sgl;
#endif
				...
				struct ump_session_data  *session_data)
{
	...
	exynos_ion_sync_dmabuf_for_device(&dev, dma_buf, dma_buf->size, DMA_BIDIRECTIONAL);
