...
static int s5m8767_tm_to_data(struct device *dev, struct rtc_time *tm, u8 *data, int set_mode)
{
	...
	if ((tm->tm_sec == 56 || tm->tm_sec == 57 ||
		...
		sec_rtc_update(info->iodev, S5M87XX_ALARM1_CONF,
...
static inline int s5m8767_rtc_set_time_reg(struct s5m_rtc_info *info)
{
	...
	ret = sec_rtc_read(info->iodev, S5M87XX_RTC_UDR_CON, &data);
	if (ret < 0)
		return ret;

	data |= (RTC_TIME_EN_MASK | RTC_UDR_MASK | RTC_UTR_T_MASK);
	data &= 0xCB;  /* 1100 1011 : SEL_OSC '0', TEST_OSC '0', SEC_FREEZE '0' */

	ret = sec_rtc_write(info->iodev, S5M87XX_RTC_UDR_CON, (char)data);
...
static inline int s5m8767_rtc_set_alarm_reg(struct s5m_rtc_info *info)
{
	...
	data |= (RTC_UDR_MASK | RTC_UTR_T_MASK);
	data &= 0xCB;  /* 1100 1011 : SEL_OSC '0', TEST_OSC '0', SEC_FREEZE '0' */
...
static int s5m_rtc_read_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = sec_rtc_bulk_read(info->iodev, S5M87XX_RTC_SEC, 8, data);
	if (ret < 0)
		return ret;
...
static int s5m_rtc_set_time(struct device *dev, struct rtc_time *tm)
{
	...
	ret = sec_rtc_bulk_write(info->iodev, S5M87XX_RTC_SEC, 8, data);
    if (ret < 0)
        return ret;
...
static int s5m_rtc_read_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
	...
	ret = sec_rtc_bulk_read(info->iodev, S5M87XX_ALARM0_SEC, 8, data);
	if (ret < 0)
		return ret;
	...
	case S5M8763X:
		...
		ret = sec_rtc_read(info->iodev, S5M87XX_ALARM0_CONF, &val);
		if (ret < 0)
			return ret;

		alrm->enabled = !!val;

		ret = sec_rtc_read(info->iodev, S5M87XX_RTC_STATUS, &val);
		if (ret < 0)
			return ret;
...
static int s5m_rtc_stop_alarm(struct s5m_rtc_info *info)
{
	...
	switch (info->iodev->device_type) {
	case S5M8763X:
		ret = sec_rtc_write(info->iodev, S5M87XX_ALARM0_CONF, 0);
		break;
	...
	case S5M8767X:
		...
		ret = sec_rtc_bulk_write(info->iodev, S5M87XX_ALARM0_SEC, 8, data);
		if (ret <0)
			return ret;
...
static int s5m_rtc_start_alarm(struct s5m_rtc_info *info)
{
	...
	case S5M8763X:
		alarm0_conf = 0x77;
		ret = sec_rtc_write(info->iodev, S5M87XX_ALARM0_CONF, alarm0_conf);
		break;
...
static void s5m_rtc_enable_wtsr(struct s5m_rtc_info *info, bool enable)
{
	...
	ret = sec_rtc_update(info->iodev, S5M87XX_WTSR_SMPL_CNTL, val, mask);
	if (ret < 0) {
...
static void s5m_rtc_enable_smpl(struct s5m_rtc_info *info, bool enable)
{
	...
	val = 0;
	sec_rtc_read(info->iodev, S5M87XX_WTSR_SMPL_CNTL, &val);
...
static int s5m8767_rtc_init_reg(struct s5m_rtc_info *info)
{
	...
	ret = sec_rtc_read(info->iodev, S5M87XX_RTC_UDR_CON, &tp_read);
	if (ret < 0) {
	...
	ret = sec_rtc_bulk_write(info->iodev, S5M87XX_ALARM0_CONF, 2, data);
	if (ret < 0) {
	...
	ret = sec_rtc_update(info->iodev, S5M87XX_RTC_UDR_CON,
...
static int __devinit s5m_rtc_probe(struct platform_device *pdev)
{
	...
	case S5M8763X:
		info->irq = pdata->irq_base + S5M8763_IRQ_ALARM0;
		break;

	case S5M8767X:
		info->irq = pdata->irq_base + S5M8767_IRQ_RTCA1;
		break;
