...
static int shmem_xmit_boot(struct shmem_link_device *shmd, struct io_device *iod,
				unsigned long arg)
{
	...
	bl_buff = shm_request_region(shm_get_phys_base(), SZ_8K);
	if (!bl_buff)
		mif_err("Allocation fail for bootloader image!!!\n");

	if (mf.size <= 0 && mf.size > SZ_8K) {
		mif_err("Unexpected cp bootloader size : 0x%x\n", mf.size);
		goto exit;
	}

	err = copy_from_user(bl_buff, (void __user *)mf.binary, mf.size);
	if (err) {
