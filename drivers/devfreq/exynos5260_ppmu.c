...
static int exynos5260_ppmu_notifier_list_init(void)
{
	int i;
...
static int exynos5260_ppmu_notify(enum DEVFREQ_TYPE type)
{
...
static void exynos5260_update_polling(unsigned int period)
{
	mutex_lock(&exynos_ppmu_lock);
...
static void exynos5260_ppmu_update(void)
{
...
int exynos5260_ppmu_activate(void)
{
	int i;
...
int exynos5260_ppmu_deactivate(void)
{
	int i;

	exynos5260_update_polling(0);
...
static int exynos5260_ppmu_reset(void)
{
...
static void exynos5260_monitor(struct work_struct *work)
{
	int i;

	mutex_lock(&exynos_ppmu_lock);

	exynos5260_ppmu_update();

	for (i = 0; i < DEVFREQ_TYPE_COUNT; ++i)
		exynos5260_ppmu_notify(i);

	exynos5260_ppmu_reset();
...
static int exynos5260_ppmu_probe(struct platform_device *pdev)
{
	exynos_ppmu_wq = create_freezable_workqueue("exynos5260_ppmu_wq");
	INIT_DELAYED_WORK(&exynos_ppmu_work, exynos5260_monitor);
	exynos5260_ppmu_activate();

	return 0;
}

static int exynos5260_ppmu_remove(struct platform_device *pdev)
{
	exynos5260_ppmu_deactivate();
...
static int exynos5260_ppmu_suspend(struct device *dev)
{
	exynos5260_ppmu_deactivate();
...
static int exynos5260_ppmu_resume(struct device *dev)
{
	exynos5260_ppmu_reset();
	exynos5260_update_polling(100);
...
static struct dev_pm_ops exynos5260_ppmu_pm = {
	.suspend	= exynos5260_ppmu_suspend,
	.resume		= exynos5260_ppmu_resume,
};

static struct platform_driver exynos5260_ppmu_driver = {
	.probe	= exynos5260_ppmu_probe,
	.remove	= exynos5260_ppmu_remove,
	.driver	= {
		.name	= "exynos5260-ppmu",
		.owner	= THIS_MODULE,
		.pm	= &exynos5260_ppmu_pm,
	},
};

static struct platform_device exynos5260_ppmu_device = {
	.name	= "exynos5260-ppmu",
	.id	= -1,
};

static int __init exynos5260_ppmu_early_init(void)
{
	return exynos5260_ppmu_notifier_list_init();
}
arch_initcall_sync(exynos5260_ppmu_early_init);

static int __init exynos5260_ppmu_init(void)
{
	int ret;

	ret = platform_device_register(&exynos5260_ppmu_device);
	if (ret)
		return ret;

	return platform_driver_register(&exynos5260_ppmu_driver);
}
late_initcall_sync(exynos5260_ppmu_init);

static void __exit exynos5260_ppmu_exit(void)
{
	platform_driver_unregister(&exynos5260_ppmu_driver);
	platform_device_unregister(&exynos5260_ppmu_device);
}
module_exit(exynos5260_ppmu_exit);
