 ...
 *	This version supports EXYNOS5420 only. This changes bus frequencies.
 *
	...
	unsigned long long *pmcnt)
{
	...
	int ret;
	unsigned long long max_ccnt = 0, temp_ccnt = 0;
	unsigned long long temp_pmcnt = 0;
	...
	*pmcnt = temp_pmcnt;
	...
	if (max_ccnt > 0) {
		busy = div_u64(temp_pmcnt, max_ccnt);
	} else {
		busy = -EINVAL;
	}
...
void exynos5_ppmu_trace(void)
{
	...
	exynos5_ppmu_handle_clear(exynos5_ppmu_trace_handle, PPMU_END);
