...
static struct sec_irq_data s2mps14_irqs[] = {
	[S2MPS14_IRQ_PWRONF] = {
		.reg = 1,
	...
	[S2MPS14_IRQ_LDO3_TSD] = {
		.reg = 3,
		.mask = S2MPS14_IRQ_LDO3_TSD_MASK,
	},
	[S2MPS14_IRQ_B16_TSD] = {
		.reg = 3,
		.mask = S2MPS14_IRQ_B16_TSD_MASK,
	},
	[S2MPS14_IRQ_B24_TSD] = {
		.reg = 3,
		.mask = S2MPS14_IRQ_B24_TSD_MASK,
	},
	[S2MPS14_IRQ_B35_TSD] = {
		.reg = 3,
		.mask = S2MPS14_IRQ_B35_TSD_MASK,
	},
};

static struct sec_irq_data s2mpa01_irqs[] = {
	[S2MPA01_IRQ_PWRONF] = {
		.reg = 1,
...
irq_to_sec_pmic_irq(struct sec_pmic_dev *sec_pmic, int irq)
{
	...
	case S2MPS14:
		return &s2mps14_irqs[irq - sec_pmic->irq_base];
	case S2MPS11X:
	...
	case S2MPA01:
		return &s2mpa01_irqs[irq - sec_pmic->irq_base];
	default:
		return NULL;
	}
}
...
static irqreturn_t sec_pmic_irq_thread(int irq, void *data)
{
	...
	case S5M8767X:
		...
		break;
	case S2MPS14:
		for (i = 0; i < S2MPS14_IRQ_NR; i++) {
			if (irq_reg[s2mps14_irqs[i].reg - 1] & s2mps14_irqs[i].mask)
				handle_nested_irq(sec_pmic->irq_base + i);
		}
		break;
	case S2MPS11X:
	...
	case S2MPU01A:
		...
		break;
	case S2MPA01:
		for (i = 0; i < S2MPA01_IRQ_NR; i++) {
			if (irq_reg[s2mpa01_irqs[i].reg - 1] & s2mpa01_irqs[i].mask)
				handle_nested_irq(sec_pmic->irq_base + i);
		}
		break;
	default:
