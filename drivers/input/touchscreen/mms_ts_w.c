...
const u8 *tmp, int len)
{
	...
	if (memcmp(buf, tmp , len)) {
		dev_info(&info->client->dev, "Run-time config verify failedn");
		mms_reboot(info);
		mms_config_set(info);
		ret = 1;
		count++;
	...
	if (count > 20) {
		mms_reboot(info);
		dev_info(&info->client->dev, "Run-time config failedn");
		ret = 1;
	}
	return ret;
}
...
const u8 len, char *text)
{
	...
	if (i2c_transfer(client->adapter, &msg, 1) != 1) {
		dev_err(&client->dev, "failed to transfer %s datan", text);
		mms_reboot(info);
		mms_config_set(info);
		return 0;
		} else {
...
static void mms_config_set(void *context)
{
	...
	if (info->config_fw == NULL) {
		dev_err(&client->dev, "failed to get config fwn");
		return;
	}
	...
	if ((conf_hdr->core_version & 0xff) != info->fw_ic_ver[1]) {
		mms_reboot(info);
		dev_err(&client->dev, "mfsp-version is not correct : 0x%x 0x%x :: 0x%x 0x%xn",
	...
	if (conf_hdr->mark[3] != 0x02) {
		mms_reboot(info);
		dev_err(&client->dev, "failed to mfsp-version : %xn",
		conf_hdr->mark[3]);
		return;
	}
	...
	for (i = 0;; i++, offset += MMS_MFSP_OFFSET) {
		...
		dev_info(&info->client->dev, "Runtime Conf_Ver[%02x]n",
		conf_item[i]->value);
...
static int mms_config_get(struct mms_ts_info *info, u8 mode)
{
	...
	if (mode == REQ_FW) {
		...
		if (system_rev == W_B2_REV00) {
			...
			if (!info->config_fw) {
				dev_err(&client->dev, "%s: Failed to allocate new requesetn", __func__);
				return -ENOMEM;
			}
		...
		if (IS_ERR(fp)) {
			dev_err(&client->dev, "file %s open error:%dn",
			fw_path, (s32)fp);
			return -EINVAL;
			} else {
			...
			fsize, &fp->f_pos);
			if (nread != fsize)
			dev_err(&client->dev, "nread != fsize errorn");
			filp_close(fp, current->files);
		}
		set_fs(old_fs);
		} else {
		dev_err(&client->dev, "%s error mode[%d]n", __func__, mode);
		return -EINVAL;
	}
	dev_info(&client->dev, "succeed to get runtime-config firmwaren");
	return 0;
}
static int mms_config_start(struct mms_ts_info *info)
{
	...
	client = info->client;
	dev_info(&client->dev, "runtime-config firmware update start!n");
...
static int mms_config_finish(struct mms_ts_info *info)
{
	...
	dev_info(&client->dev, "succeed to runtime-config firmware updaten");
	return 0;
}
static int mms_isc_read_status(struct mms_ts_info *info, u32 val)
{
	...
	if (!cnt) {
		dev_err(&client->dev,
		"status read fail. cnt : %d, val : 0x%x != 0x%xn",
		cnt, result, val);
	}
	return ret;
}
	...
	for (page = img->start_page; page <= img->end_page; page++) {
		...
		for (i = 0; i < ISC_BLOCK_NUM; i++, ptr += ISC_XFER_LEN) {
			...
			ISC_XFER_LEN)) {
				...
				false);
				#endif
				dev_err(&client->dev, "flash verify failedn");
				ret = -1;
				goto out;
			}
		}
	}
	dev_info(&client->dev, "section [%d] update succeededn", img->type);
	ret = 0;
	goto out;
	i2c_err:
	dev_err(&client->dev, "i2c failed @ %sn", __func__);
	ret = -1;
	out:
	kfree(isc_packet);
	kfree(msg[1].buf);
...
u8 update_mode)
{
	...
	if (retires < 0) {
		dev_warn(&client->dev, "failed to obtain ver. infon");
	...
	i++, offset += sizeof(struct mms_fw_img)) {
		...
		if (update_mode == FORCED_UPDATE) {
			update_flag = true;
			dev_info(&client->dev, "section [%d] forced update mode. ver : 0x%x, bin : 0x%xn",
			img[i]->type, ver[img[i]->type],
			target[i]);
			} else if (ver[img[i]->type] == 0xff) {
			update_flag = true;
			dev_info(&client->dev, "tsp ic fw error : section [%d] is need to be updatedn",
			img[i]->type);
			/* defence code for empty tsp ic */
			} else if (ver[img[i]->type] > 0x30) {
			update_flag = true;
			dev_info(&client->dev, "empty tsp ic : section [%d] is need to be updatedn",
			img[i]->type);
			} else if (ver[img[i]->type] < target[i]) {
			update_flag = true;
			dev_info(&client->dev, "section [%d] is need to be updated. ver : 0x%x, bin : 0x%xn",
			img[i]->type, ver[img[i]->type],
			target[i]);
			} else {
			dev_info(&client->dev, "section [%d] is up to date, ver : 0x%xn",
			i, target[i]);
		}
		if (update_flag) {
			if (!isc_enter_flag) {
			...
			if (ret) {
				dev_err(&client->dev,
				"failed to mms_flash_section[%d]n", i);
				mms_reboot(info);
				goto out;
			}
		}
	...
	if (isc_enter_flag) {
		...
		mms_reboot(info);
		if (get_fw_version_ic(client, ver)) {
			dev_err(&client->dev, "failed to obtain version after flashn");
			ret = -1;
			goto out;
			} else {
			for (i = 0; i < fw_hdr->section_num; i++) {
				if (ver[img[i]->type] != target[i]) {
					dev_info(&client->dev,
					"version mismatch after flash. [%d] 0x%02x != 0x%02xn",
					i, ver[img[i]->type],
					target[i]);
					ret = -1;
					goto out;
...
static void work_mms_config_set(struct work_struct *work)
{
	...
	mms_config_set(info);
	#if TSP_TA_CALLBACK
	dev_info(&info->client->dev, "%s & TA %sconnect & noise mode %s!n",
	__func__,
	info->ta_status ? "" : "dis",
...
static void change_dvfs_lock(struct work_struct *work)
{
	...
	"%s dev change bud lock failed(%d)n",
	__func__, __LINE__);
	else
...
static void set_dvfs_off(struct work_struct *work)
{
	...
	if (ret < 0)
	dev_err(&info->client->dev, " %s: cpu unlock failed(%d)n",
	__func__, __LINE__);
...
{
	int i;
	dev_notice(&info->client->dev, "%sn", __func__);
	...
	dev_notice(&info->client->dev, "dvfs lock free.n");
	#endif
}
static void reset_mms_ts(struct mms_ts_info *info)
{
	...
	#if TSP_TA_CALLBACK
	dev_info(&client->dev,
	"%s & tsp %s & TA %sconnect & noise mode %s!n",
	__func__,
	info->enabled ? "on" : "off",
	info->ta_status ? "" : "dis",
	info->noise_mode ? "on" : "off");
...
static void melfas_ta_cb(struct tsp_callbacks *cb, bool ta_status)
{
	...
	dev_info(&client->dev, "%s & tsp %s & TA %sconnect & noise mode %s!n",
	__func__,
	info->enabled ? "on" : "off",
	info->ta_status ? "" : "dis",
	info->noise_mode ? "on" : "off");
	...
	if (info->ta_status) {
		...
		if (info->noise_mode) {
			i2c_smbus_write_byte_data(info->client,
			MMS_NOISE_REG, MMS_NOISE_ON);
			dev_err(&client->dev, "melfas_ta_cb & abnormal noise mode on!n");
		}
		} else {
		...
		if (info->noise_mode) {
			...
			dev_info(&client->dev, "ta_cb & noise mode off!n");
		}
	}
	#endif
}
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	if (sz < 0) {
		...
		if (i == 50) {
			dev_dbg(&client->dev, "i2c failed... reset!!n");
			reset_mms_ts(info);
			return IRQ_HANDLED;
		}
		dev_err(&client->dev, "success read touch info datan");
	}
	if (sz == 0)
	return IRQ_HANDLED;
	if (sz > MAX_FINGERS*finger_event_sz || sz%finger_event_sz) {
		dev_err(&client->dev, "abnormal data inputed & reset IC[%d]n",
		sz);
		reset_mms_ts(info);
		return IRQ_HANDLED;
	}
	...
	if (ret != ARRAY_SIZE(msg)) {
		dev_err(&client->dev,
		"failed to read %d bytes of touch data (%d)n",
		sz, ret);
		for (i = 0; i < 5; i++) {
		...
		if (i == 5) {
			dev_err(&client->dev,
			"failed to read touch data & reset IC[%d]n",
			ret);
			reset_mms_ts(info);
			return IRQ_HANDLED;
		}
		dev_err(&client->dev, "success read touch datan");
	}
	if (buf[0] == MMS_NOTIFY_EVENT) {	/* MMS_NOTIFY_EVENT */
		dev_info(&client->dev, "TSP noise mode enter!(%d)n", buf[1]);
		i2c_smbus_write_byte_data(info->client,
		MMS_NOISE_REG, MMS_NOISE_ON);
		if (!info->ta_status)
		dev_err(&client->dev, "TSP noise mode enter but ta off!n");
		info->noise_mode = true;
		return IRQ_HANDLED;
	}
	if (buf[0] == MMS_ERROR_EVENT) { /* MMS_ERROR_EVENT*/
		dev_err(&client->dev, "Error detected, restarting TSPn");
		reset_mms_ts(info);
		return IRQ_HANDLED;
	}
...
static int mms_ts_fw_load(struct mms_ts_info *info)
{
	...
	if (!info->pdata) {
		dev_err(&client->dev,
		"fw cannot be updated, missing platform datan");
		return 1;
	}
	/* firmware update */
	do {
	...
	if (!retries) {
		dev_err(&info->client->dev, "failed to flash firmware after retiresn");
		} else {
...
static void get_intensity_data(struct mms_ts_info *info)
{
	...
	return;
	err_i2c:
	dev_err(&info->client->dev, "%s: fail to i2c (cmd=%d)n",
...
static void get_raw_data(struct mms_ts_info *info, u8 cmd)
{
	...
	while (retry) {
		...
		if (ret < 0)
		goto err_i2c;
		dev_info(&info->client->dev, "event type = 0x%xn", r_buf);
		if (r_buf == 0x0C)
		retry = 0;
	}
	ret = i2c_smbus_read_i2c_block_data(info->client,
	...
	if (ret < 0)
	goto err_i2c;
	dev_info(&info->client->dev, "get_data : r_buf = 0x%xn", r_buf);
	w_buf[0] = ADDR_UNIV_CMD;
	...
	goto err_i2c;
	if (r_buf == 1)
	dev_info(&info->client->dev, "PASSn");
	else
	dev_info(&info->client->dev, "FAILn");
	if (cmd == MMS_VSC_CMD_CM_DELTA)
...
struct device_attribute *attr, char *buf)
{
	...
	info->ft_flag = 0;
	return snprintf(buf, TSP_BUF_SIZE, "%un", 0);
}
...
static int check_rx_tx_num(void *device_data)
{
	...
	info->cmd_param[1] >= info->pdata->tsp_rx) {
		...
		set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
		info->cmd_state = 3;
		dev_info(&info->client->dev, "%s: parameter error: %u,%un",
	...
	dev_info(&info->client->dev, "%s: node = %dn", __func__,
	node);
	return node;
}
static void not_support_cmd(void *device_data)
{
	...
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = 4;
	dev_info(&info->client->dev, "%s: %s(%d)n", __func__,
...
static void fw_update(void *device_data)
{
	...
	set_default_result(info);
	dev_info(&client->dev,
	"fw_ic_ver = 0x%02x, fw_bin_ver = 0x%02xn",
	info->fw_ic_ver[SEC_CONF], info->pdata->fw_bin_ver[SEC_CONF]);
	switch (info->cmd_param[0]) {
		case BUILT_IN:
		info->cmd_param[0] = REQ_FW;
		dev_info(&client->dev, "BUILT_IN=>REQ_FW update mode!n");
		break;
		case UMS:
		dev_info(&client->dev, "UMS update mode!n");
		break;
		case REQ_FW:
		dev_info(&client->dev, "REQ_FW update mode!n");
		break;
		default:
		dev_err(&client->dev, "invalid cmd_param[%d]n",
		info->cmd_param[0]);
		goto not_support;
	}
	disable_irq(info->irq);
	...
	dev_err(&client->dev, "fail fw updaten");
	ret = mms_config_get(info, info->cmd_param[0]);
	if (ret) {
		dev_err(&client->dev, "fail mms_config_getn");
		mms_config_get(info, REQ_FW);
	}
	mms_config_set(info);
	...
	for (i = 0; i < MAX_SECTION_NUM; i++)
	info->fw_ic_ver[i] = ver[i];
	dev_info(&client->dev, "After FW update : [0x%02x]n", ver[SEC_CONF]);
	enable_irq(info->irq);
	if (fw_read_flag && !ret) {
...
static void get_config_ver(void *device_data)
{
	...
	"config-set-conf-ver");
	if (!ret)
	dev_err(&info->client->dev, "mms_config_flash fail!n");
	config_flash_set[0] = MMS_GET_RUN_CONF;
	mms_read_config(info->client, config_flash_set, &config_ver, 1);
	dev_info(&info->client->dev, "Runtime config_ver [%02x]n", config_ver);
	set_default_result(info);
...
static void module_off_master(void *device_data)
{
	...
	info->cmd_state = FAIL;
	dev_info(&info->client->dev, "%s: %sn", __func__, buff);
}
static void module_on_master(void *device_data)
{
	struct mms_ts_info *info = (struct mms_ts_info *)device_data;
...
static void get_cm_abs(void *device_data)
{
	...
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
	info->cmd_state = OK;
	dev_info(&info->client->dev, "%s: %s(%d)n", __func__, buff,
...
static void get_x_num(void *device_data)
{
	...
	char buff[16] = {0};
	if (info->enabled)
	dev_info(&info->client->dev, "%s = [%d] from icn", __func__,
...
*devattr, const char *buf, size_t count)
{
	...
	if (info->cmd_is_running == true) {
		dev_err(&info->client->dev, "tsp_cmd: other cmd is running.n");
		goto err_out;
	}
	/* check lock  */
	...
	if (cur && cmd_found) {
		...
		do {
			if (*cur == delim || cur - buf == len) {
				...
				*(buff + strlen(buff)) = ' ';
				ret = kstrtoint(buff, 10,info->cmd_param + param_cnt);
				start = cur + 1;
				memset(buff, 0x00, ARRAY_SIZE(buff));
				param_cnt++;
			}
	...
	dev_info(&client->dev, "cmd = %sn", tsp_cmd_ptr->cmd_name);
	for (i = 0; i < param_cnt; i++)
	dev_info(&client->dev, "cmd param %d= %dn", i,
	info->cmd_param[i]);
...
struct device_attribute *devattr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	char buff[16] = {0};
	dev_info(&info->client->dev, "tsp cmd: status:%dn",
	info->cmd_state);
	...
	return snprintf(buf, TSP_BUF_SIZE, "%sn", buff);
}
static ssize_t show_cmd_result(struct device *dev, struct device_attribute
*devattr, char *buf)
{
	struct mms_ts_info *info = dev_get_drvdata(dev);
	dev_info(&info->client->dev, "tsp cmd: result: %sn", info->cmd_result);
	mutex_lock(&info->cmd_lock);
	info->cmd_is_running = false;
	mutex_unlock(&info->cmd_lock);
	info->cmd_state = WAITING;
	return snprintf(buf, TSP_BUF_SIZE, "%sn", info->cmd_result);
}
...
static int melfas_power(struct mms_ts_info *info, int onoff)
{
	...
	if (!vddo_vreg) {
		...
		if (IS_ERR(vddo_vreg)) {
			vddo_vreg = NULL;
			dev_err(&info->client->dev,
			"[TSP] failed to get vddo regulatorn");
			return -ENODEV;
		}
	}
	if (!avdd_vreg) {
		...
		if (IS_ERR(avdd_vreg)) {
			avdd_vreg = NULL;
			dev_err(&info->client->dev,
			"[TSP] failed to get avdd regulatorn");
			return -ENODEV;
		}
		rc = regulator_set_voltage(avdd_vreg, 3300000, 3300000);
		if (rc) {
			dev_err(&info->client->dev,
			"[TSP] unable to set voltage for avdd_vregn");
			return rc;
		}
	}
	if (onoff) {
		if (regulator_is_enabled(vddo_vreg)) {
			dev_err(&info->client->dev,
			"[TSP] vddo is already enabledn");
			} else {
			rc = regulator_enable(vddo_vreg);
			if (rc) {
				dev_err(&info->client->dev,
				"[TSP] unable to enable vddon");
				return rc;
			}
		}
		if (regulator_is_enabled(avdd_vreg)) {
			dev_err(&info->client->dev,
			"[TSP] avdd is already enabledn");
			} else {
			rc = regulator_enable(avdd_vreg);
			if (rc) {
				dev_err(&info->client->dev,
				"[TSP] unable to enable avddn");
				return rc;
			}
		}
		} else {
		if (regulator_is_enabled(vddo_vreg)) {
			...
			if (rc) {
				dev_err(&info->client->dev,
				"[TSP] unable to disable vddon");
				return rc;
			}
			} else {
			dev_err(&info->client->dev,
			"[TSP] vddo is already disabledn");
		}
		if (regulator_is_enabled(avdd_vreg)) {
			rc = regulator_disable(avdd_vreg);
			if (rc) {
				dev_err(&info->client->dev,
				"[TSP] unable to disable avddn");
				return rc;
			}
			} else {
			dev_err(&info->client->dev,
			"[TSP] avdd is already disabledn");
		}
	}
...
{
	pr_debug("[TSP] melfas_register_callback not implemented yetn");
}
...
const struct i2c_device_id *id)
{
	...
	if (!input_dev) {
		dev_err(&client->dev, "Failed to allocate memory for input devicen");
		ret = -ENOMEM;
	...
	if (client->dev.of_node) {
		...
		if (ret) {
			dev_err(&client->dev, "Error parsing dt %dn", ret);
			return ret;
		}
	...
	if (NULL == info->pdata) {
		dev_err(&client->dev, "failed to get platform datan");
		goto err_config;
	}
	info->irq = -1;
	...
	if (ret) {
		dev_err(&client->dev, "failed to initialize (%d)n", ret);
		goto err_fw_init;
	}
	info->enabled = true;
	info->resume_done = true;
	...
	*/
	...
	if (ret) {
		dev_err(&client->dev, "failed to register input dev (%d)n",
		ret);
	...
	dev_info(&client->dev,
	"Melfas MMS-series touch controller initializedn");
	#if SEC_TSP_FACTORY_TEST
	...
	dev_err(&client->dev, "Failed to create device for the sysfsn");
	ret = sysfs_create_group(&fac_dev_ts->kobj,
	&sec_touch_factory_attr_group);
	if (ret)
	dev_err(&client->dev, "Failed to create sysfs groupn");
	#endif	/* SEC_TSP_FACTORY_TEST */
	pr_info("[TSP] %s donen", __func__);
	return 0;
	#if SEC_TSP_FACTORY_TEST
	err_alloc_node_data_failed:
	dev_err(&client->dev, "Err_alloc_node_data failedn");
...
#if (defined(CONFIG_PM) || defined(CONFIG_HAS_EARLYSUSPEND)) && !defined(ALWAYS_ON_TOUCH)
static int mms_ts_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mms_ts_info *info = i2c_get_clientdata(client);
	int retries = 50;
	dev_info(&info->client->dev, "%s %sn", __func__,
	info->enabled ? "ON" : "OFF");
