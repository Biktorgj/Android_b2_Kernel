...
#define PROC_DIR	"tssettings/"
struct proc_dir_entry *ts_dir;
int procFSTouchWake = 1;
/* Biktor: To allow touchwake */
int hasAlreadyStarted=0;
...
static int mms_ts_input_open(struct input_dev *dev)
{
	...
	if (hasAlreadyStarted==0)
		...
		if (info->enabled) {
			dev_info(&info->client->dev,"%s already power on\n", __func__);
		return 0;
		}
	wake_lock(&info->wake_lock);
	...
	info->enabled = true;
		...
		printk ("melfas: Touchscreen was already on\n");
	}
	return 0;
}
...
{
		/* 			Testing!
		PM:SLEEP shouldn't shutdown the TS, 
		otherwise touch.wake won't work and
		we'll have to use the power button
		to bring up the display */
//printk ("melfas: Touch Wake status %i - hasAlreadyStarted? %i", procFSTouchWake, hasAlreadyStarted);
if (!procFSTouchWake)
	hasAlreadyStarted=0;
...
#endif
/* ProcFS Touch Wake config */
static ssize_t melfas_read_proc_touchwake(struct file *file, char __user *userbuf,size_t bytes, loff_t *off)
{
	return procFSTouchWake;
}

static ssize_t melfas_write_proc_touchwake(struct file *file, const char __user *buffer,size_t count, loff_t *pos)
{
	if (buffer[0]=='1')
		procFSTouchWake=1;
	else
...
static const struct file_operations proc_fops_touchwake = {
	.owner = THIS_MODULE,
	.read = melfas_read_proc_touchwake,
	.write = melfas_write_proc_touchwake,
};
				  ...
				  const struct i2c_device_id *id)
{
	...
	int tx_num;
#endif
	ts_dir = proc_mkdir("tssettings", NULL);
	if (ts_dir == NULL) {
		printk("melfas Error:Unable to create /proc/tssettings directory");
		return -ENOMEM;
	}
	ent = proc_create("touchwake", 0, ts_dir, &proc_fops_touchwake);
	if (ent == NULL) {
		printk("melfas Error:Unable to create /proc/%s/touchwake entry", PROC_DIR);
		ret = -ENOMEM;
		goto fail;
	}
