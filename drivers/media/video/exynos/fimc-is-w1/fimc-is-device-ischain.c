	...
	size_t buf_len, loff_t *ppos)
{
	...
	vb2_dma_contig_memops.finish(imemory->dc_buf);
#else
...
int fimc_is_ischain_power(struct fimc_is_device_ischain *device, int on)
{
	...
	if (on) {
		...
		timeout = 1000;
		while ((readl(PMUREG_ISP_STATUS) & 0x7) && timeout) {
			timeout--;
		...
		if (timeout == 0) {
			...
			err("  EXYNOS3_CLKGATE_BUS_ISP0(0x%08x)\n",readl(EXYNOS3_CLKGATE_BUS_ISP0));
			err("  EXYNOS3_CLKGATE_BUS_ISP1(0x%08x)\n",readl(EXYNOS3_CLKGATE_BUS_ISP1));
			err("  EXYNOS3_CLKGATE_BUS_ISP2(0x%08x)\n",readl(EXYNOS3_CLKGATE_BUS_ISP2));
			err("  EXYNOS3_CLKGATE_BUS_ISP3(0x%08x)\n",readl(EXYNOS3_CLKGATE_BUS_ISP3));
			err("  EXYNOS3_CLKGATE_IP_ISP0(0x%08x)\n",readl(EXYNOS3_CLKGATE_IP_ISP0));
			err("  EXYNOS3_CLKGATE_IP_ISP1(0x%08x)\n",readl(EXYNOS3_CLKGATE_IP_ISP1));
			err("  EXYNOS3_CLKSRC_MASK_ISP(0x%08x)\n",readl(EXYNOS3_CLKSRC_MASK_ISP));
			err("  EXYNOS3_CLKGATE_SCLK_ISP(0x%08x)\n",readl(EXYNOS3_CLKGATE_SCLK_TOP_ISP));
			err("  EXYNOS3_CLKGATE_SCLK_ISP(CMU_ISP)(0x%08x)\n",readl(EXYNOS3_CLKGATE_SCLK_ISP));

			err("  EXYNOS3_ISP_OPTION(0x%08x)\n",readl(EXYNOS3_ISP_OPTION));
			err("  EXYNOS3_CMU_CLKSTOP_ISP_SYS_PWR_REG(0x%08x)\n",readl(EXYNOS3_CMU_CLKSTOP_ISP_SYS_PWR_REG));
			err("  EXYNOS3_CMU_SYSCLK_ISP_SYS_PWR_REG(0x%08x)\n",readl(EXYNOS3_CMU_SYSCLK_ISP_SYS_PWR_REG));
			err("  EXYNOS3_CMU_RESET_ISP_SYS_PWR_REG(0x%08x)\n",readl(EXYNOS3_CMU_RESET_ISP_SYS_PWR_REG));
			err("  PMUREG_ISP_ARM_SYS_PWR_REG(0x%08x)\n",readl(PMUREG_ISP_ARM_SYS_PWR_REG));
			err("  PMUREG_ISP_ARM_OPTION(0x%08x)\n",readl(PMUREG_ISP_ARM_OPTION));
			err("  PMUREG_ISP_CONFIGURATION(0x%08x)\n",readl(PMUREG_ISP_CONFIGURATION));
			err("  PMUREG_ISP_STATUS(0x%08x)\n",readl(PMUREG_ISP_STATUS));
	...
	struct fimc_is_video_ctx *vctx)
{
	...
	} else if (GET_FIMC_IS_NUM_OF_SUBIP(core, 3a0)) {
		/* 1. Stop all request */
...
#endif
	set_bit(FIMC_IS_SENSOR_SKIP_SENSOR_MODE, &sensor->state);
	if (ret) {
	...
	struct fimc_is_queue *queue)
{
	...
	if ((test_bit(FIMC_IS_ISCHAIN_OPEN_SENSOR, &device->state)) &&
		(!test_bit(FIMC_IS_SENSOR_SKIP_SENSOR_MODE, &device->sensor->state)))
