...
struct pm_qos_request exynos_isp_qos_cpu;
#define TIZEN_CAM_CPU_LOCK 400000
...
static int fimc_is_ischain_allocmem(struct fimc_is_core *this)
{
	...
	buf = vb2_dma_contig_memops.alloc(this->mem.alloc_ctx,
				FIMC_IS_A5_MEM_SIZE +
#ifdef ENABLE_ODC
	...
	this->minfo.dc_buf = buf;

	this->minfo.dvaddr = *(dma_addr_t *)vb2_dma_contig_memops.cookie(buf);

	if ((ret < 0) || (this->minfo.dvaddr  & FIMC_IS_FW_BASE_MASK)) {
		err("The base memory is not aligned to 64MB.");
		vb2_dma_contig_memops.put(buf);
		this->minfo.dvaddr = 0;
		ret = -EIO;
		goto exit;
	}
	...
	this->minfo.kvaddr = (u32)vb2_dma_contig_memops.vaddr(buf);
...
int fimc_is_runtime_suspend(struct device *dev)
{
	...
	pm_qos_remove_request(&exynos_isp_qos_cpu);
#endif
...
int fimc_is_runtime_resume(struct device *dev)
{
	...
	pm_qos_add_request(&exynos_isp_qos_cpu, PM_QOS_CPU_FREQ_MIN, TIZEN_CAM_CPU_LOCK);
	pr_info("[RSC] %s: QoS LOCK [CPU(%d)]\n", __func__, TIZEN_CAM_CPU_LOCK);
...
/* Tizen: Add camera sysfs node for camera sensor information */
static ssize_t s5k8b1_camera_camtype_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char type[] = "SLSI_S5K8B1_FIMC_IS";

	return sprintf(buf, "%s\n", type);
}

static ssize_t s5k8b1_camera_camfw_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	char type[] = "S5K8B1";
...
static DEVICE_ATTR(rear_camtype, S_IRUGO,
		s5k8b1_camera_camtype_show, NULL);
static DEVICE_ATTR(rear_camfw, S_IRUGO, s5k8b1_camera_camfw_show, NULL);

static int fimc_is_probe(struct platform_device *pdev)
{
	struct class *camera_class;	/* sys/class/camerar */
	...
	if (IS_ERR(camera_class)) {
		err("failed to create device cam_dev_rear!\n");
		return 0;
	}
	...
	if (IS_ERR(rear_dev)) {
		...
		if (device_create_file(rear_dev, &dev_attr_rear_camtype) < 0) {
			err("failed to create device file, %s\n",
