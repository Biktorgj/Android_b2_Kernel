			   ...
			   struct v4l2_capability *cap)
{
	...
	strncpy(cap->driver, jpeg->plat_dev->name, sizeof(cap->driver) - 1);
	strncpy(cap->card, jpeg->plat_dev->name, sizeof(cap->card) - 1);
	cap->bus_info[0] = 0;
			     ...
			     struct v4l2_format *f)
{
	...
	if (f->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		jpeg_hx_get_frame_size(ctx->jpeg_dev->reg_base, &width, &height);
