...
crc_retry:
	...
	fimc_is_i2c_read(core->client0, cal_buf, 0x0, FIMC_IS_MAX_CAL_SIZE);

	sysfs_finfo.oem_start_addr = *((u32 *)&cal_buf[0x0]);
	sysfs_finfo.oem_end_addr = *((u32 *)&cal_buf[0x04]);
	pr_info("OEM start = 0x%08x, end = 0x%08x\n",
			(sysfs_finfo.oem_start_addr), (sysfs_finfo.oem_end_addr));
	sysfs_finfo.awb_start_addr = *((u32 *)&cal_buf[0x08]);
	sysfs_finfo.awb_end_addr = *((u32 *)&cal_buf[0x0C]);
	pr_info("AWB start = 0x%08x, end = 0x%08x\n",
			(sysfs_finfo.awb_start_addr), (sysfs_finfo.awb_end_addr));
	sysfs_finfo.shading_start_addr = *((u32 *)&cal_buf[0x10]);
	sysfs_finfo.shading_end_addr = *((u32 *)&cal_buf[0x14]);
	...
	if (write_data_to_file(FIMC_IS_CAL_SDCARD, cal_buf,
 ...
 */
static int __used fimc_is_sec_ldo_enabled(struct device *dev, char *name) {
	struct regulator *regulator = NULL;
	int enabled = 0;
                ...
                char *fw_name, char *setf_name, int isSysfsRead)
{
	...
	if ((!sysfs_finfo.is_caldata_read && (cam_id == CAMERA_SINGLE_REAR)) ||
	    force_caldata_dump) {
		is_dumped_fw_loading_needed = false;
		if (force_caldata_dump)
			pr_info("forced caldata dump!!\n");

		if (!fimc_is_sec_ldo_enabled(dev, "cam_sensor_io_1v8")) {
			ret = fimc_is_sec_ldo_enable(dev, "cam_sensor_io_1v8", true);
			if (ret) {
	...
	snprintf(fw_path, sizeof(fw_path), "%s%s", FIMC_IS_FW_PATH, FIMC_IS_FW);
...
exit:
	if (is_ldo_enabled) {
		ret = fimc_is_sec_ldo_enable(dev, "cam_sensor_io_1v8", false);
		if (ret)
...
int fimc_is_sec_fw_sel(struct device *dev,
	char *fw_name, char *setf_name, int isSysfsRead)
{
	int ret = 0;
#if 1
