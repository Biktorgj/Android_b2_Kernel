...
static inline dma_addr_t s5p_mfc_mem_plane_addr(
	struct s5p_mfc_ctx *c, struct vb2_buffer *v, unsigned int n)
{
	return (dma_addr_t)vb2_dma_contig_plane_dma_addr(v, n);
}

static inline void *s5p_mfc_mem_alloc_priv(void *alloc_ctx, size_t size)
{
	return vb2_dma_contig_memops.alloc(alloc_ctx, size, 0, 0);
}

static inline void s5p_mfc_mem_free_priv(void *vb_priv)
{
	vb2_dma_contig_memops.put(vb_priv);
}

static inline dma_addr_t s5p_mfc_mem_daddr_priv(void *vb_priv)
{
	dma_addr_t *paddr = vb2_dma_contig_memops.cookie(vb_priv);
	return *paddr;
}

static inline void *s5p_mfc_mem_vaddr_priv(void *vb_priv)
{
	return vb2_dma_contig_memops.vaddr(vb_priv);
}
...
static inline void s5p_mfc_mem_clean_priv(void *vb_priv, void *start,
					off_t offset, size_t size)
{
	vb2_dma_contig_memops.prepare(vb_priv);
}

static inline void s5p_mfc_mem_inv_priv(void *vb_priv, void *start,
					off_t offset, size_t size)
{
	vb2_dma_contig_memops.finish(vb_priv);
}
#endif
