...
static int s5k4ecgx_power(struct v4l2_subdev *sd, int on)
{
	...
	CHECK_ERR_COND_MSG((S5K4ECGX_HW_POWER_NOT_READY == pdata->power),
			-EPERM, "power %d not ready\n", on)

	if (on) {
		if (S5K4ECGX_HW_POWER_ON == pdata->power)
			cam_err("power already on\n");

		err = pdata->power_on(pdata);
		CHECK_ERR_MSG(err, "fail to power on %d\n", err);

		pdata->power = S5K4ECGX_HW_POWER_ON;
	} else {
		if (S5K4ECGX_HW_POWER_OFF == pdata->power)
    ...
    || defined(CONFIG_VIDEO_FAST_MODECHANGE_V2)
		...
		pdata->power = S5K4ECGX_HW_POWER_OFF;
		state->initialized = 0;
	}
...
#endif /* CONFIG_MACH_GARDA || CONFIG_MACH_WICHITA*/
