...
#include "board-b2-fpga-fw.h"

#define DUMMY_BIT_COUNT	49

#define GPIO_IRDA_IRQ		EXYNOS3_GPX1(0)   /*U24*/
#define GPIO_CRESET_B		EXYNOS3_GPX2(5)   /*W21*/
#define GPIO_CDONE		EXYNOS3_GPX2(4)   /*Y24*/
#define GPIO_FPGA_RST_N		EXYNOS3_GPE0(6)   /*D14*/
#define GPIO_IRDA_SCL		EXYNOS3_GPA1(3)   /*C22*/
#define GPIO_IRDA_SDA		EXYNOS3_GPA1(2)   /*D19*/
...
static struct platform_device *b2_fpga_devices[] __initdata = {
	&s3c_device_i2c22,
};
...
static int fpga_write_configuration(void)
{
	...
	ret = gpio_request_one(GPIO_CDONE, GPIOF_IN, "IRDA_CDONE");
	if (ret < 0) {
		pr_err("%s: GPIO (IRDA_CDONE) request failed: %d\n",
				__func__, ret);
		goto err_irda_cdone;
	}
...
#if 1
void __init exynos3_b2_fpga_init(void)
{
	...
	platform_add_devices(b2_fpga_devices,
			ARRAY_SIZE(b2_fpga_devices));		
}
