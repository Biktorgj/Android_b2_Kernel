...
/* TODO: Need to fix after charger bringup */
...
/* TODO: Need to fix after charger bringup */
		...
		if ((is_cable_attached || poweroff_try >= 5) && (!lpcharge)) {
			pr_emerg
			    ("%s: charger connected(%d) or power"
			     "off failed(%d), reboot!\n",
			     __func__, is_cable_attached, poweroff_try);
#else /* Not CONFIG_MACH_B2 */
		if (poweroff_try >= 5) {
			pr_emerg
			    ("%s: poweroff failed(%d), reboot!\n",
			     __func__, poweroff_try);
#endif /* CONFIG_MACH_B2 */
