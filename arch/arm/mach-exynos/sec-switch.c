...
void max77803_muic_usb_cb(u8 usb_mode)
{
	...
	struct usb_gadget *gadget =
		platform_get_drvdata(&s3c_device_usb_hsotg);
	int g_state;
#endif
	...
	if (usb_mode == USB_CABLE_ATTACHED) {
		...
		if (g_state < 0) {
			pr_err("%s:gadget_vbus disconnect failed(%d)\n",
				__func__, g_state);
		}
#endif
...
//#if 0 //CHECK ME!!
	...
	if (ret < 0)
		...
		pr_info("%s: MUIC usb_switch registered\n", __func__);
#endif
//#endif //CHECK ME!!
}
...
//	} else {
		...
		regulator = regulator_get(NULL, "safeout");
		if (IS_ERR(regulator))
			return -ENODEV;
		if (!regulator_is_enabled(regulator))
			regulator_enable(regulator);
