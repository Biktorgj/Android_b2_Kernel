...
static void s3c_adc_hw_enable(struct adc_device *adc);
static void s3c_adc_hw_disable(struct adc_device *adc);
...
static void s3c_adc_hw_deinit(struct adc_device *adc)
{
	enum s3c_cpu_type cpu = platform_get_device_id(adc->pdev)->driver_data;
	unsigned long value;

	if (cpu != TYPE_ADCV5) {
		value = readl(adc->regs + S3C2410_ADCCON);
		value |= S3C2410_ADCCON_STDBM;
		writel(value, adc->regs + S3C2410_ADCCON);
	} else {
		value = readl(adc->regs + SAMSUNG_ADC2_CON1);
		value &= ~SAMSUNG_ADC2_CON1_STC_EN;
		writel(value, adc->regs + SAMSUNG_ADC2_CON1);

		value = readl(adc->regs + SAMSUNG_ADC2_CON2);
		value &= ~(SAMSUNG_ADC2_CON2_OSEL | SAMSUNG_ADC2_CON2_ESEL |
			SAMSUNG_ADC2_CON2_HIGHF | SAMSUNG_ADC2_CON2_C_TIME(0));
		writel(value, adc->regs + SAMSUNG_ADC2_CON2);
		writel(0, adc->regs + SAMSUNG_ADC2_INT_EN);
	}
}

static void s3c_adc_hw_disable(struct adc_device *adc)
{
	struct s3c_adc_platdata *pdata = adc->pdev->dev.platform_data;
	unsigned long flags;
	...
	s3c_adc_hw_deinit(adc);
	clk_disable(adc->clk);

	if (pdata != NULL && pdata->phy_init != NULL && adc->clk_cnt == 0)
		pdata->phy_exit();
