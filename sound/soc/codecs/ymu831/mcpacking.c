...
#define	MCDRV_WAIT_TIME_2US		(2UL)
...
#define	RTC_PLLMODE	(3)
#define	RTC_PLLPREVDIV	(1)
#define	RTC_PLLFBDIV	(4500)
#define	RTC_PLLFRAC	(0)
#define	RTC_PLLFREQ	(1)

SINT32	WakeANACLK_PD(
	UINT8	*pbRegPD,
	UINT8	*pbAP
)
{
...
#if (MCDRV_DEBUG_LEVEL >= 4)
	McDebugLog_FuncIn("WakeANACLK_PD");
#endif

	McResCtrl_GetInitInfo(&sInitInfo, &sInit2Info);

	if ((*pbRegPD&MCB_PLL_PD) != 0) {
		if ((*pbAP&MCB_AP_LDOD) != 0) {
			*pbAP	&= (UINT8)~(MCB_AP_LDOD|MCB_AP_BGR);
			McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
					| MCDRV_PACKET_REGTYPE_ANA
					| (UINT32)MCI_AP,
					*pbAP|MCDRV_AP_LDOD);
		...
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				| MCDRV_PACKET_REGTYPE_A
				| (UINT32)MCI_PLL_FBDIV_B_12_8,
				(UINT8)(RTC_PLLFBDIV>>8));
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				| MCDRV_PACKET_REGTYPE_A
				| (UINT32)MCI_PLL_FBDIV_B_7_0,
				(UINT8)(RTC_PLLFBDIV&0xFF));
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				| MCDRV_PACKET_REGTYPE_A
				| (UINT32)MCI_PLL_FRAC_B_15_8,
				(UINT8)(RTC_PLLFRAC>>8));
...
exit:
#if (MCDRV_DEBUG_LEVEL >= 4)
	McDebugLog_FuncOut("WakeANACLK_PD", &sdRet);
#endif
	return sdRet;
}
...
)
{
	...
	&& ((bAP & MCB_AP_LDOA) == 0)) {
		...
		McDevIf_AddPacket(MCDRV_PACKET_TYPE_WRITE
				...
				bAP);
		McDevIf_AddPacket(
			MCDRV_PACKET_TYPE_TIMWAIT | MCDRV_WAIT_TIME_2US, 0);
		sdRet	= McDevIf_ExecutePacket();
		if (sdRet != MCDRV_SUCCESS) {
			goto exit;
		}
